...
<p class="search-result">
 ...
 if ($found >= '4') {$fathin="narrow";}else{$fathin="widen";}
    ...
    echo "<h3>We found ".$found . $result_txt ." for <strong>'$s'</strong>";
    if ($cat != '') {
        echo " in '$cat'.</h3><p> Use the drop down box below to ".$fathin." your search to particular areas of interest.</p>";
    }else{echo " across the whole site</h3><p>This includes both information pages <strong>and</strong> news stories. </p><p>Use the drop down box below to ".$fathin." your search to particular areas of interest.</p>";}
   ...
   <form role="search" method="get" class="search-form extrasearch" action="<?php echo site_url(); ?>/">
	...
	<input type="text" class="search-field" value="<?php the_search_query(); ?>" name="s" id="s" /></div>
      <div class="search-cats">  <label for "cat">Search in:</label> 
		<?php
    wp_dropdown_categories(array('show_option_all' => 'Whole site','taxonomy' => 'category','hierarchical' =>'1','hide_empty' => '1','include' => '2,4,5,151,240,238,239,247,242,244,243')); ?></div>
